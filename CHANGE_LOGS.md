...
- Env var renames: `RCLONE_MANAGER_DOMAIN` replaces individual `SUBDOMAIN_*` vars; `RCLONE_MANAGER_PROJECT_NAME` is the compose project/network prefix (required)
- `TAILSCALE_CLIENT_SECRET` → `RCLONE_MANAGER_TAILSCALE_AUTHKEY` (standardised Tailscale env naming)
- `RCLONE_MANAGER_APP_PORT` now drives the app container port directly; `SUBDOMAIN_APP`, `SUBDOMAIN_DOZZLE`, etc. removed
...
- **`compose.apps.yml`** — parameterised app service (`RCLONE_MANAGER_APP_IMAGE` + `RCLONE_MANAGER_APP_PORT`)
...
- Subdomain auto-convention: all routes derived from `${RCLONE_MANAGER_PROJECT_NAME}.${RCLONE_MANAGER_DOMAIN}` pattern
- `RCLONE_MANAGER_DC_VERBOSE=1` debug flag for `docker-compose/scripts/dc.sh`
- `RCLONE_MANAGER_HEALTH_PATH` env to customise healthcheck endpoint per image

### Changed

- Image versions pinned (caddy `2.9.1-alpine`, cloudflared `2025.1.0`, dozzle `v8.x`, filebrowser `v2.30.0`, tailscale `stable`)
- Caddy `CADDY_INGRESS_NETWORKS` now uses `${RCLONE_MANAGER_PROJECT_NAME}_net` (was `app_net`)
- Network name: `${RCLONE_MANAGER_PROJECT_NAME:-mystack}_net` (dynamic, avoids conflicts between stacks)
...
- `TAILSCALE_CLIENT_SECRET` (use `RCLONE_MANAGER_TAILSCALE_AUTHKEY`)
- Hardcoded `build: ./services/app` in compose (now `RCLONE_MANAGER_APP_IMAGE` param)
